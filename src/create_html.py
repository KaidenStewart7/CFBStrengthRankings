...
class CreateHTML():
        ...
        with open(strength_ratings_path, "r") as file:
            ...
            for line in lines:
                if count != 0:
                    ...
                    self.teams.append({"Name": name, "Rating": float(rating).__round__(4)})
                else:
                    count += 1
    ...
    def print_index_file(self):
        with open(index_path, "w") as file:
            ...
            file.write("    <head>\n")
            file.write("        <title>College Football Strength Ratings</title>\n")
            file.write("        <meta charset=\"UTF-8\" >\n")
            file.write("    </head>\n")
            file.write("    <body>\n")
            file.write("        <h1>College Football Strength Ratings</h1>\n")
            file.write("        <p>\n")
            ...
            while week_count > 0:
                ...
                file.write(f"            <li><a href=\"{week_path}\">Week {week_count} Rankings</a></li>\n")
                week_count -= 1
            file.write("        </ul>\n")
            file.write("    </body>\n")
            file.write("</html>\n")
    ...
    def print_weekly_files(self):
        ...
        with open(week_path, "w") as file:
            ...
            file.write("<html lang=\"en\">\n")
            file.write("    <head>\n")
            file.write(f"        <title>College Football Strength Ratings - Week {week_count}</title>\n")
            file.write("        <meta charset=\"UTF-8\" >\n")
            file.write("    </head>\n")
            file.write("    <body>\n")
            file.write(f"        <h1>College Football Strength Ratings - Week {week_count}</h1>\n")
            file.write("        <ol>\n")
            count = 0
            while count < 25:
                team = self.teams[count]
                file.write(f"            <li><span>{team['Name']}</span> <span>{team['Rating']}</span></li>\n")
                count += 1
            file.write("        </ol>\n")
            file.write("        <p>\n")
